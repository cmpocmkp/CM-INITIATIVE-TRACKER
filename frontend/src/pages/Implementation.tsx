...
export default function Implementation() {
  ...
  const rows = useMemo(() => {
    ...
    for (const s of schemes ?? []) {
      ...
      // No separate executing agency means the owner department implements it itself.
      const key = agency || s.department?.name || "Unassigned";
      const type: Agg["type"] = agency ? "agency" : "department";
      const a = by.get(key) ?? { key, type, count: 0, cost: 0, alloc: 0, spent: 0, physW: 0, w: 0 };
      a.count++;
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {rows.map((a) => {
          ...
          return (
            <Link
              key={a.key}
              to={`/implementation/${encodeURIComponent(a.key)}`}
              className="card group p-4 transition hover:border-white/30 hover:shadow-md"
            >
              <h3 className="truncate text-[14px] leading-snug text-white/95" title={a.key}>
                {a.key}
              </h3>
              <div className="mt-1 text-[11px] text-white/50">
                {a.type === "agency" ? "implementing agency" : "department · self-implemented"}
              </div>
